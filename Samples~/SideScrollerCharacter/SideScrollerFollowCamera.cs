...
{
    /// <summary>
    /// A minimal 2D orthographic follow camera for the sample: each <c>LateUpdate</c> it finds the side-scroller
    /// character entity in the default ECS world, reads its <see cref="LocalToWorld"/> translation, and smoothly
    /// follows it in X/Y (the camera stays on its own Z so it looks down the +Z axis at the 2D plane). A plain
    ...
    ///
    /// <para>Drop it on the scene's Camera (set Orthographic). It is inert until a baked character exists, so it does
    ...
    /// ECS subject.</para>
    /// </summary>
    [AddComponentMenu("Zori/Entities Character Controller 2D/Samples/Side-Scroller Follow Camera")]
    [RequireComponent(typeof(Camera))]
    public sealed class SideScrollerFollowCamera : MonoBehaviour
    {
        [SerializeField]
        ...
        void TryBindWorld()
        {
            ...
            if (_world is { IsCreated: true })
            {
                _characterQuery = _world.EntityManager.CreateEntityQuery(
                    ComponentType.ReadOnly<SideScrollerCharacterTag>(),
        ...
        void LateUpdate()
        {
            ...
            // Follow the first side-scroller character (the sample has one). ToComponentDataArray copies the matches;
            // for the single-character sample this is one element.
